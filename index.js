...
const tableContainerWrapper = document.getElementById('table-container-wrapper');
const tableContainer = document.getElementById('table-container');
const tableTitle = document.getElementById('table-title');
const quoteContainer = document.getElementById('quote-container');
const quoteTextElement = document.querySelector('#quote-container .quote-text');
const quoteAuthorElement = document.querySelector('#quote-container .quote-author');
 ...
 */
function renderOutput(labels, cumulativeInvestmentData, simulationData, noAnimation = false) {
...
function renderSummaries(simulationData) {
 ...
 */
function renderChart(labels, cumulativeInvestmentData, simulationData, noAnimation = false) {
    ...
    const datasets = sortedReturns.map(returnValue => {
        const data = simulationData.get(returnValue);
        const color = getColorForReturn(returnValue);
...
function getColorForReturn(returnValue) {
    const customReturnInput = document.getElementById('custom-return-rate');
...
// --- Event Handling ---
function updateStateAndRunSimulation(event) {
    ...
    const btcToggle = document.getElementById('bitcoin-toggle');
    ...
    const getRadioOrCustomValue = (groupName, customInputId) => {
    ...
    returnCheckboxes.forEach(checkbox => {
        state.selectedReturns.push(parseFloat(checkbox.value));
    });
    const customReturnInput = document.getElementById('custom-return-rate');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    radioGroups.forEach(name => {
        ...
        const customInput = document.querySelector(`.custom-toggle-input[data-group="${name}"]`);
        if (customInput) {
    ...
    controlsContainer.addEventListener('click', (event) => {
        ...
        const input = wrapper.querySelector('.custom-toggle-input');
